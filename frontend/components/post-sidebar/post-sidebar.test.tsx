...
import { createPostBodyModel, type BlogPostSidebar } from "./model";
...
describe("PostSidebar", () => {
  it("renders all four CMS actions in document order", () => {
    ...
    expect(
      within(sidebar).getByRole("heading", { name: "Explore the topic" }),
    ).toBeInTheDocument();
    expect(
      within(sidebar)
        .getAllByRole("heading", { level: 3 })
        .map((item) => item.textContent),
    ).toEqual(["Read the guide", "External resource", "Checklist", "Contact"]);
  });

  it("points click-to-edit at Blog Post Settings fields", () => {
    const dataAttribute = vi.fn((path: string) => `source:${path}`);
    render(
      <PostSidebar dataAttribute={dataAttribute} sidebar={currentSidebar} />,
    );

    expect(
      screen.getByRole("complementary", { name: "Post actions" }),
    ).toHaveAttribute("data-sanity", "source:actions");
    expect(
      screen.getByRole("heading", { name: "Explore the topic" }),
    ).toHaveAttribute("data-sanity", "source:title");
    expect(dataAttribute).not.toHaveBeenCalledWith(
      expect.stringContaining("blogPostSidebar."),
    );
  });
  ...
  it("renders exactly two CMS actions in document order", () => {
    render(
      <PostSidebar
        sidebar={{
          ...currentSidebar,
          actions: currentSidebar.actions.slice(0, 2),
        }}
      />,
    );
    ...
    expect(
      screen
        .getAllByRole("heading", { level: 3 })
        .map((item) => item.textContent),
    ).toEqual(["Read the guide", "External resource"]);
  });
  ...
  it("places action descriptions before their links", () => {
    ...
    const description = screen.getByText(
      "Open a related resource in a new tab.",
    );
  ...
  it("matches an ID for a valid heading even when its Portable Text key is absent", () => {
    ...
    expect(
      screen.getByRole("link", { name: "Unkeyed heading" }),
    ).toHaveAttribute("href", "#unkeyed-heading");
